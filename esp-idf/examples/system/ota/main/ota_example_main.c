...
/* fftust: to test user partition */
STATIC void mb_user_partition_test_t()
{
  const esp_partition_t *user_partition = NULL;
  
  ESP_LOGI(TAG, "makeblock: user partition test start\n");
  esp_err_t ff_ret = 0;
  uint8_t ff_test[256];
  ...
  ESP_LOGI(TAG, "init read ret is %d ,data[15] is: %d \n", ff_ret,ff_test[15]);
  
  memset(ff_test, 10 , 256);
  ff_ret = esp_partition_erase_range(user_partition, 0, user_partition->size);
  ESP_LOGI(TAG, "erase ret is %d", ff_ret);
  ff_ret = esp_partition_write(user_partition, 0, ff_test, 100);
  ESP_LOGI(TAG, "write ret is %d \n", ff_ret);	
  ff_ret = esp_partition_read(user_partition, 0, ff_test, 100);
  ESP_LOGI(TAG, "second read ret is %d ,data[15] is: %d \n", ff_ret,ff_test[15]);
  
  
  
  ESP_LOGI(TAG, "makeblock: user partition test end\n");
...
void app_main()
{
    ...
    ESP_ERROR_CHECK( err );
    ESP_LOGI(TAG, "fftust: factory update ota_0");
    initialise_wifi();
