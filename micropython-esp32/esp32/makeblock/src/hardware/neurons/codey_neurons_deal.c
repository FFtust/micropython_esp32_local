 ...
 ******************************************************************************/
static SemaphoreHandle_t s_recv_data_sem;
...
void codey_neurons_deal(void * parameter)
{
  ...
  while(1)
  {
    if(xSemaphoreTake(s_recv_data_sem, MAX_DEAL_TIME_MS / portTICK_PERIOD_MS) == pdTRUE)
    {
      ble_more_data = true;
      ...
      while(ble_more_data || uart0_more_data || uart1_more_data)
      {
        ...
        if(0 == strcmp("NEU_FTY_READY", s_rx_file_name))
        {
          ...
          ESP_LOGD(TAG, "start receive new script");
        }  
        else if(0 == strcmp(MAIN_PY_SCRIPT_NAME, s_rx_file_name))
        {
          ESP_LOGD(TAG, "received new script and set restart flag");
